layui.define(['table', 'form'], function(exports) {
    ...
    table.render({
        id:"studentinfoTableContainer",
        elem: '#LAY-studentinfo-manage' //渲染id=myTable的表格
        ,url: '/studentinfo/queryStudentList' //数据接口 //访问控制器，获得表格所需的数据 !!!!
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'sid', title: 'ID', width:180, sort: true}
            ,{field: 'sname', title: '学生名称', width:180,sort:true,edit:'text',style:'color:green'}
    ...
    //监听搜索
    form.on('submit(LAY-studentinfo-search)', function(data){
        var field = data.field;//表单元素值
        console.log(field);
        //执行重载
        table.reload('studentinfoTableContainer', {
    ...
    //监听表格上方工具按钮的事件
    table.on('toolbar(LAY-studentinfo-manage)', function(obj){
        ...
        switch(obj.event){
            case 'add':
                ...
                saveType ='add';
                //把studentinfo/edit.html页面显示在后面管理的弹出层中
                admin.popup({
                    title: '添加用户'
                    ,area: ['500px', '450px']
                    ,id: 'LAY-popup-studentinfo-add'
                    ,success: function(layero, index){ //此处的index是弹出层的编号，可以做为layer.close(index)的参数关闭弹出层;
                        view(this.id).render('studentinfo/edit', null).done(function(){
                            ...
                            //监听保存按钮事件
                            form.on("submit(studentinfoFrom)", function(data) {
            ...
            case 'update':
                ...
                if (checkStatus.data.length!=1){
                    ...
                    console.log("-----------------------------");
                    console.log(rowData);
                    //  lay-filter="studentinfoFormFilter"
                    admin.popup({
                        title: '编辑用户'
                        ,area: ['500px', '450px']
                        ,id: 'LAY-popup-studentinfo-edit'
                        ,success: function(layero, index){ //此处的index是弹出层的编号，可以做为layer.close(index)的参数关闭弹出层;
                            view(this.id).render('studentinfo/edit', null).done(function(){
                                form.val("studentinfoFormFilter",rowData);
                                ...
                                form.on("submit(studentinfoFrom)", function(data) {
                                    ...
                                    admin.req(
                                        {
                                            url: '/studentInfo/editStudentName' ,
                                            data:data.field,
                                            success:function(serverData){//服务 器处理成功的回调函数，服务端返回的数据放在serverData变量中
                                                console.log("服务端返回的数据为:"+serverData);
                                                console.log(serverData);
                    ...
                    form.val("studentinfoFormFilter",rowData);  //studentinfoFormFilter 即 class="layui-form" 所在元素属性 lay-filter="" 对应的值
                    //给表单赋值元素赋值
    ...
    //监听工具条
    table.on('tool(LAY-studentinfo-manage)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            layer.confirm('真的删除行么', function(index){
                ...
                $.get("/studentInfo/deleteStdentu",{},function(serverData){
