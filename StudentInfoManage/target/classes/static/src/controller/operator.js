layui.define(['table', 'form'], function(exports) {
    ...
    table.render({
        ...
        elem: '#LAY-operator-manage' //渲染id=myTable的表格
        ...
        ,cols: [[ //表头
            ...
            ,{  title: '所属权限组', width:180, sort: true,templet: '<div>{{d.roleGroupBeans.groupname}}</div>'}// 表示找到页面中的id=groupNames的元素，在元素内部用layui的模板引擎把json数据取出来显示
            ,{field: 'lastLoginIp', title: '最后登录时间', width:180, sort: true}
            ,{field: 'lastLoginTime', title: '最后登录ip', width:180, sort: true}
    ...
    //监听搜索
    form.on('submit(LAY-operator-search)', function(data){
        var field = data.field;//表单元素值
        //执行重载
        table.reload('operatorTableContainer', {
    ...
    //监听表格上方工具按钮的事件
    table.on('toolbar(LAY-operator-manage)', function(obj){
        ...
        switch(obj.event){
            case 'add':
                ...
                admin.popup({
                    title: '添加用户'
                    ,area: ['500px', '450px']
                    ,id: 'LAY-popup-operator-add'
                    ,success: function(layero, index){ //此处的index是弹出层的编号，可以做为layer.close(index)的参数关闭弹出层;
                        view(this.id).render('operator/edit',null).done(function(){
                                ...
                                //监听保存按钮事件
                                form.on("submit(addOperatorForm)", function(data) {
                                    console.log(data);
                                    ...
                                    admin.req(
                                        {
                                            url: 'http://localhost:8080/operator/add' ,
                                            data:data.field,
                                            success:function(serverData){//服务 器处理成功的回调函数，服务端返回的数据放在serverData变量中
                                                console.log("服务端返回的数据为:"+serverData);
                                                console.log(serverData);
                    ...
                    admin.popup({
                        ...
                        ,success: function(layero,index){ //此处的index是弹出层的编号，可以做为layer.close(index)的参数关闭弹出层;
                            view(this.id).render('operator/edit',null).done(function(){
                                ...
                                form.val("operatorFormFilter",{
                                    "loginname":rowData.loginname
                                    ,"oldname":rowData.loginname
                                })
                                //监听保存按钮事件
                                form.on("submit(addOperatorForm)", function(data) {
                                    console.log(data);
                                    ...
                                    admin.req(
                                        {
                                            url: 'http://localhost:8080/operator/edit' ,
                                            data:data.field,
                                            success:function(serverData){//服务 器处理成功的回调函数，服务端返回的数据放在serverData变量中
                                                console.log("服务端返回的数据为:"+serverData);
                                                console.log(serverData);
