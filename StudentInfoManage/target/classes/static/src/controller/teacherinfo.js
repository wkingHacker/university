layui.define(['table', 'form'], function(exports) {
    ...
    table.render({
        id:"teacherinfoTableContainer",
        elem: '#LAY-teacherinfo-manage' //渲染id=myTable的表格
        ,url: '/teacherInfo/list' //数据接口 //访问控制器，获得表格所需的数据
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'tname', title: '教师名', width:180,sort:true,edit:'text',style:'color:green'}
            ,{field: 'sex',title: '性别', width:180, sort: true}
            //templet: '#sex' 表示找到页面中的sex的元素，在元素内部用layui的模板引擎把json数据取出来显示
    ...
    //监听搜索
    form.on('submit(LAY-teacherinfo-search)', function(data){
        var field = data.field;//表单元素值
        //执行重载
        table.reload('teacherinfoTableContainer', {
    ...
    //监听表格上方工具按钮的事件
    table.on('toolbar(LAY-teacherinfo-manage)', function(obj){
        var data = obj.data;
        ...
        switch(obj.event){
            case 'add':
                saveType ='add';
                //把teacherinfo/edit.html页面显示在后面管理的弹出层中
                admin.popup({
                    title: '添加用户'
                    ,area: ['500px', '450px']
                    ,id: 'LAY-popup-teacherinfo-add'
                    ,success: function(layero, index){ //此处的index是弹出层的编号，可以做为layer.close(index)的参数关闭弹出层;
                        view(this.id).render('teacherinfo/edit', null).done(function(){

                            //监听保存按钮事件
                            form.on("submit(addteacherinfoFrom)", function(data) {
            ...
            case 'update':
                ...
                if (checkStatus.data.length!=1){
                    ...
                    admin.popup({
                        title: '添加用户'
                        ,area: ['500px', '450px']
                        ,id: 'LAY-popup-teacherinfo-edit'
                        ,success: function(layero,index){ //此处的index是弹出层的编号，可以做为layer.close(index)的参数关闭弹出层;
                            view(this.id).render('teacherinfo/edit',null).done(function(){
                                form.val("teacherinfoFormFilter",{
                                    "tid":rowData.tid,
                                    "sex":rowData.sex,
                                ...
                                form.on("submit(addteacherinfoFrom)", function(data) {
                                    ...
                                    admin.req(
                                        {
                                            url: 'http://localhost:8080/teacherInfo/edit' ,
                                            data:data.field,
                                            success:function(serverData){//服务 器处理成功的回调函数，服务端返回的数据放在serverData变量中
                                                console.log("服务端返回的数据为:"+serverData);
                                                console.log(serverData);
    ...
    //监听工具条
    table.on('tool(LAY-teacherinfo-manage)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
